...
void ConvertModel::onInit(){
  ...
  detection_per_class_ = net_ptr_->getOutputScoreSize()[0]; //检测个数
  class_num_ = net_ptr_->getOutputScoreSize()[1];

  cv::Mat img = cv::imread(image_file);
  std::cout << "reading figure is over, push back to in_imgs: " << image_file << std::endl;
...
void ConvertModel::infer(){
  ...
  while (num_rois != 0) {
    ...
    //  std::vector<cv::Mat> cropped_imgs;
      ...
      std::cout << "cvMat2CnnInput is over" << std::endl;
      //copy to gpu
  ...
  const int num_rois, std::vector<Detection> & detections)
{
  ...
  for (int i = 0; i < num_rois; ++i) {
    ...
    std::cout << i+1 << " detection: " << "prob= " << det.prob << ",x=" << det.x << ",y=" << det.y << ",w=" << det.w << ",h=" << det.h << std::endl;
  }
  return true;
}
  ...
  for (int i = 0; i < num_rois; ++i) {
    ...
    for (int c = 0; c < channel_; ++c) {
      for (int j = 0, hw = width_ * height_; j < hw; ++j) {
        // std::cout << "begin = c=" << c << " j=" << j << " hw=" << hw << std::endl;
        // std::cout << "data.size: " << data.size() << std::endl;
        // std::cout << "w=" << width_ << ", h=" << height_ << std::endl;
        // std::cout << "index= " << i * channel_ * width_ * height_ + c * hw + j << std::endl;
        // std::cout << "data[index]=" << data[i * channel_ * width_ * height_ + c * hw + j] << std::endl;
        // std::cout << "img.size=" << img.size() << std::endl;
        // std::cout << "mean_[c]=" << mean_[c] << std::endl;
        // std::cout << "img[channel_ * j + 2 - c]=" << img[channel_ * j + 2 - c] << std::endl;
